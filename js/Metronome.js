...
RING.Metronome = Backbone.Model.extend({
	name : "Metronome",
	...
	initialize : function(attributes, options) {
		...
		this.scriptNode = RING.context.createJavaScriptNode(2048, 1, 1);
		this.scriptNode.connect(RING.context.destination);
		//setup the callback
	...
	createScriptCallback : function() {
		...
		this.scriptNode.onaudioprocess = function(event) {
			if(!self.get("stopped")) {
				//timing
				var playbackTime = RING.context.currentTime;
				var samplesToSeconds = 1 / RING.context.sampleRate;
				//process samples
